...
const cacheSubscriptionRates = async () => {
    ...
    fs.writeFile(__dirname + "/../cached_data/sub_rates.json", JSON.stringify(rateObjArr), (err) => {if(err) console.log(err)});
...
const cacheSubscriptionCoupons = async () => {
    ...
    fs.writeFile(__dirname + "/../cached_data/sub_coupons.json", JSON.stringify(couponObjArr), (err) => {if(err) console.log(err)});
