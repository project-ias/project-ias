...
fs.readFile('pyq_scrapers/pyqs.json', async (err, data) => {
    ...
    for(let i = 0; i < array_length; i++) {
        ...
        for (let topic of json_data[i]['topics']) {
            if(topic.includes(";")) { // eg - "Historical underpinnings & evolution; Features, amendments, significant provisions, basic structure; Comparison of Indian constitutional scheme with other countries’"
                let split_arr = topic.split(";")
                seperated_topics = seperated_topics.concat(split_arr)
            } else if(topic.includes(",")) { // eg -"Salient aspects of Art, Architecture, literature from Ancient to Modern Times"
                let split_arr = topic.split(',')
                seperated_topics = seperated_topics.concat(split_arr)
            } else {
